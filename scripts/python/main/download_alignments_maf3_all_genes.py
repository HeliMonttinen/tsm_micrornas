...
def locus_ids2(data_file):
    """
    Collects unique identifiers for each
    given locus. Reasoning for this is
    that one locus may contain multiple
    identifiers.
    ...
    with open(data_file, 'r') as f:
        for line in f:
            ...
            chromosome = line_splitted[1].split(':')[0].lstrip('chr')
            start = int(line_splitted[1].split(':')[1].split('-')[0])
            end = int(line_splitted[1].split(':')[1].split('-')[1].rstrip())
            identifiers[line_splitted[0]] = [chromosome, start, end]
...
def spes_names(species_file):
    """
    Parses species names and assembly identifiers from the
    the .maf alignment associated species file downloaded
    from the UCSC database. Returns species names
    as a dict format.

    Argument
    =========

    species_file: A .maf file associated species file
                  downloaded from the UCSC database.

    Returns
    ========
    species_dict: A dictionary, in which assembly identifier
                  is a key and species name a value
...
def extract_areas(start, end, identifier, chr_file,
    ...
    for item in final:
        if item not in species_dict:
            continue
        max_ids = 0

        for item2 in final:
            if item2 not in species_dict:
                continue
        ...
        if max_ids > 0.80 and len(final[item].replace('-', '')) > 100:
            final2[item] = final[item]
        elif 'hg38' in item:
            final2[item] = final[item]

    with open(
            seq_info_dir + identifier + '.txt', '+a') as f1:

        for spes in seq_info_dict:
            if spes not in species_dict:
                continue
            if 'Homo' in species_dict[spes]:

                human_len = int(info[spes].replace(':', '_').split('_')[-1]) -\
                        int(info[spes].replace(':', '_').split('_')[-2])

        for spes in seq_info_dict:
            if spes not in species_dict:
                continue

            spes_len = int(info[spes].replace(':', '_').split('_')[-1]) -\
                int(info[spes].replace(':', '_').split('_')[-2])

            if spes_len > 2*human_len:
                continue

            if str(seq_info_dict[spes]["start_strand"]) == '+':
                start_index = info[spes].replace(':', '_').split('_')[-2]
            else:
            ...
            if str(seq_info_dict[spes]["end_strand"]) == '+':
                end_index = info[spes].replace(':', '_').split('_')[-1]
            ...
            with open(tar_dir + identifier + '.fas', 'a+') as f2:
                ...
                if spes in final2:
                    ...
                    new_header = '>' + (
                        species_dict[spes]).replace(' ', '_') +\
                        '_' + info[spes].replace(':', '_') + '\n'
                    f2.write(new_header)
                    f2.write(seq)
...
def main():
    ...
    identifiers = locus_ids2(data_file)
