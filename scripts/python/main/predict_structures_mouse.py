...
def main():
    ...
    for filename in return_file(alignment_dir):
        ...
        for seq in bio_align_dict:
            align_dict[align_id + '_' + seq.replace('_+_','').replace('_-_','')] = str(bio_align_dict[seq].seq)
            seq = align_id + '_' + seq.replace('_+_','').replace('_-_','')
        ...
        for struct in struct_list:
            try:
                ...
                analyse_structure(
                        structure_dir + struct + ".fasta",
                        structure_dir + struct + ".db",
                        "RNAfold")
            except:
                continue
