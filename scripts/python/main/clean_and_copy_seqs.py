    ...
    for subdir, dirs, files in os.walk(alignment_dir):
        for file in files:
            ...
            for key in fasta_dict:
                if 'Homo' in key:
                    Homo_sapiens = key
            if len(Homo_sapiens) == 0:
                continue

            for key in fasta_dict:
                if len(fasta_dict[key].replace('-', '')) >\
                        len(fasta_dict[Homo_sapiens].replace('-', ''))*1.3:
                    continue
                    ...
                    if 'Homo' in new_key:
                        Homo_sapiens_new = new_key

                else:
                    copy_dict[key] = fasta_dict[key]
                    if 'Homo' in key:
                        Homo_sapiens_new = key
