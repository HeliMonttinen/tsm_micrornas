...
def locus_ids(data_file):
    """
    Collects identifiers of non-overlapping loci.

    Argument
    ========
    :data_file: A data file contains the
                information on the identifier and
                locus separated by a tab. The identifier
                is located in the first column and the
                locus in the last.
    Output
    ======
    Identifiers: A dictionary in which identifiers are keys
                 and locus coordinates as a value.
                 The value is in a list format containing
                 a chromosome, starting and stopping indexes
                 [chr1, 1234, 1235]
...
def extract_areas(start, end, identifier, chr_file,
    ...
    for item in final:
        ...
        if max_ids > 0.80 and len(final[item].replace(
                '-', '')) > 30:
    ...
    with open(seq_info_dir + identifier + '.txt', '+a') as f1:
        ...
        for spes in seq_info_dict:
            ...
            if 'Homo' in species_dict[spes]:
                ...
                human_len = int(info[spes].replace(
                    ':', '_').split('_')[-1]) -\
                        int(info[spes].replace(
                            ':', '_').split('_')[-2])

        for spes in seq_info_dict:

            spes_len = int(info[spes].replace(
                ':', '_').split('_')[-1]) -\
                    int(info[spes].replace(
                        ':', '_').split('_')[-2])

            if spes_len > 2.5*human_len:
                continue
