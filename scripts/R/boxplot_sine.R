...
dist = read.table("results/distance_unmasked_tsms_variants_sine.txt")[,7]
dat = cbind.data.frame(set="tsm",dist)

for(i in 1:100){
    dist = read.table(paste0("results/genome_unmasked_corrected_tsms_distance/background_sine_",i))[,7]
...
ggsave("results/transposons_enrichment/plot_sine_tsm_corrected.pdf", image,width=4.5,height=4.5)

#pdf("results/plot_sine_tsm_corrected.pdf")
