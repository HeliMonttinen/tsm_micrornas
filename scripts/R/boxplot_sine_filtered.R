...
dist = read.table("results/distance_tsms_all_sine_filtered.txt")[,9]
dat = cbind.data.frame(set="tsm",dist)

for(i in 1:100){
    dist = read.table(paste0("results/genome_wide_background_distance_full/background_filtered_sine_",i))[,9]
...
ggsave("results/transposons_enrichment/plot_sine_all_filtered.png",width=5,height=4)
