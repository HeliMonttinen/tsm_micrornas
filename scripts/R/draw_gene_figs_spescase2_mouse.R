...
offs=50000
data <- read.table("results/mouse_mirbasedb2/list_mouse_mir.txt")
colnames(data) <- c("id","chr","start","stop", "strand")
data2 <- read.table("results/mouse_mirbasedb2/transposons_microRNA.txt",
		    sep="\t")
...
tmp_dict <- read.table("results_revision/mouse_mirbasedb2/transcript_symbols_mouse.txt", sep='\t')
...
for(i in 1:dim(data)[1]){
    ...
    idTrack <- IdeogramTrack(chromosome = chrom, genome = "mm10")
    ...
    tx = makeTxDbFromGFF("results/mouse_mirbasedb2/shortened_mouse.gff3")
    ...
    pdf(paste0("results_revision/mouse_mirbasedb2/genome_figs/",ident,"_knowngff3_.pdf"), width=10, height=7)
